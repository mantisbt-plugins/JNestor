...
public class MantisProcessorActionResendAsText implements MantisProcessorAction
{
	private static Log log = LogFactory.getLog(MantisProcessorActionResendAsText.class);
	...
	public void doAction(long issueId, MailMessage msg, boolean exists) throws JNException
	{
		try
		{
			...
			// that will be a text message
			String newBody = body; 
			newBody = newBody.replace( "##issueid", Long.toString(issueId) );
			
			// Add to the body the text of the original message
			newBody += "\n\n--------- Original Message ---------\n\n";
			newBody += String.format( "Subject: %s\n", msg.getSubject() );
			newBody += String.format( "Date: %s\n", msg.getSentDate() );
			newBody += String.format( "From: %s\n", msg.getFromWithName() );
			newBody += msg.getBodyAsText();
			
			newMsg.setText( newBody );
